export const auth = {
  login: {
    title: "Ласкаво просимо до Gustio",
    subtitle: "Швидко увійдіть або створіть акаунт для управління закладом та QR-меню.",
    emailLabel: "Електронна пошта",
    emailPlaceholder: "your.restaurant@gmail.com",
    emailHint: "Використовуйте пошту, прив'язану до вашого закладу.",
    codeLabel: "Код підтвердження",
    codePlaceholder: "123456",
    resendCode: "Вислати ще раз",
    resendCodeTimer: "Вислати ще раз через",
    continueBtn: "Продовжити",
    createAccountBtn: "Створити акаунт",
    verifyBtn: "Підтвердити код",
    termsPrefix: "Продовжуючи, ви погоджуєтеся з нашою ",
    privacyPolicy: "Політикою конфіденційності",
    and: " та ",
    terms: "Умовами використання",
    or: "або",
    google: "Продовжити з Google",
    apple: "Продовжити з Apple",
  },
  errors: {
    emailRequired: "Поле обов'язкове для заповнення",
    emailInvalid: "Некоректний формат email",
    emailCyrillic: "Email не може містити кириличні літери (А-Я)",
    codeLength: "Код має містити 6 цифр",
    codeNumeric: "Код має складатися лише з цифр",
    verifyFailed: "Невірний код або термін його дії минув",
    serverError: "Внутрішня помилка сервера",
    defaultError: "Щось пішло не так. Спробуйте ще раз."
  }
};