export const qr = {
  title: "QR-Коди та Столи",
  subtitle: "Створюйте посадкові зони, керуйте активністю столів та генеруйте QR-коди для друку.",
  emptyTitle: "У вас ще немає столів",
  emptyDesc: "Додайте перший стіл, щоб згенерувати для нього унікальний QR-код.",
  addBtn: "Додати стіл",
  printBtn: "Видрукувати QR",
  selected: "Обрано",
  selectAll: "Вибрати всі",
  table: "Стіл",
  statusActive: "Активний",
  statusInactive: "Неактивний",
  numberPlaceholder: "Наприклад: 1, 12 або 105",
  modal: {
    createTitle: "Новий стіл",
    editTitle: "Редагування столу",
    numberLabel: "Номер або назва столу",
    numberPlaceholder: "Наприклад: 1, 12 або VIP",
    typeLabel: "Локація / Зона",
    typePlaceholder: "Наприклад: Тераса, Основний зал",
    typeHint: "Почніть вводити, щоб побачити попередні зони",
    statusLabel: "Приймати замовлення з цього столу",
    createZoneLabel: "Створити нову зону / локацію",
    cancel: "Скасувати",
    newZonePlaceholder: "Назва нової зони",
    save: "Зберегти"
  },
  errors: {
    numberRequired: "Номер столу обов'язковий",
    numberUnique: "Стіл з таким номером вже існує",
    typeRequired: "Вкажіть зону або локацію"
  },
  notifications: {
    createSuccess: "Стіл успішно створено",
    updateSuccess: "Параметри столу оновлено",
    deleteSuccess: "Стіл успішно видалено"
  },
  deleteConfirm: "Ви впевнені, що хочете видалити цей стіл? Старий QR-код перестане працювати."
};