  ...
  return (
    <div className="flex h-full flex-col bg-bg-main p-6 transition-colors duration-300">
      ...
      <div className="max-w-2xl">
        <Card className="p-6 bg-bg-surface border border-border-main/60 dark:border-border-main rounded-2xl shadow-table flex flex-col md:flex-row gap-6 items-center md:items-start">
          ...
          <div className="flex-1 w-full space-y-4">
            <div>
              <h2 className="text-2xl font-bold text-text-main text-center md:text-left">
                {user.firstName || user.lastName ? `${user.firstName ?? ''} ${user.lastName ?? ''}`.trim() : t('profile.defaultUser')}
              </h2>
              <span className="inline-block mt-1 px-3 py-1 bg-brand-emerald/10 text-brand-emerald text-xs font-bold rounded-full uppercase tracking-wider">
