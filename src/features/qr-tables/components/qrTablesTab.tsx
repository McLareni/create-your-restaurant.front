...
export const QrTablesTab = () => {
  ...
  const selectBaseClasses = "h-12 w-full rounded-md border bg-white px-3 py-2 text-sm text-brand-espresso outline-none transition-colors focus:border-brand-copper focus:ring-1 focus:ring-brand-copper";

  return (
    <div className="flex h-full flex-col">
      <div className="flex items-center justify-between mb-6 pb-4 border-b border-brand-gray/10 print:hidden shrink-0">
        <div>
          <h2 className="text-xl font-semibold text-brand-espresso">{t('qr.title')}</h2>
          <p className="text-sm text-brand-gray mt-1">{t('qr.subtitle')}</p>
        </div>
        ...
        {tables.length === 0 ? (
          ...
          <div className="flex flex-col h-full">
            <div className="mb-4 flex items-center gap-2 px-1 shrink-0">
              ...
              <span className="text-sm font-medium text-brand-espresso">{t('qr.selectAll')}</span>
            </div>
      ...
      <div className="hidden print:flex flex-wrap gap-8 justify-center items-center w-full bg-white">
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={editingTable ? t('qr.modal.editTitle') : t('qr.modal.createTitle')}>
        <div className="flex flex-col gap-5">
          ...
          <div className="flex flex-col gap-1.5">
            <label htmlFor="type" className="text-sm font-medium text-brand-espresso">{t('qr.modal.typeLabel')}</label>
          ...
          <div className="flex items-center justify-between border-t border-brand-gray/10 pt-4">
            <span className="text-sm font-medium text-brand-espresso">{t('qr.modal.statusLabel')}</span>
