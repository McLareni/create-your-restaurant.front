...
import { useComboModal } from '../../hooks/combos/useComboModal';
import { Button, FloatingPanel, Input, Select, Checkbox } from '@/shared/ui';
import { ComboModalProps } from '../../types/combos.types';

export const ComboModal = ({ isOpen, onClose, isLoading, onSave, initialData }: ComboModalProps) => {
  const state = useComboModal(initialData, onSave, onClose);
  ...
  return (
    <FloatingPanel
      ...
      isOpen={isOpen}
      onClose={onClose}
      title={initialData?.name ? state.t('menu.constructor.combos.modal.editTitle') : state.t('menu.constructor.combos.modal.createTitle')}
      ...
      <div className="flex flex-col text-brand-espresso dark:text-brand-cream w-full h-162.5 max-h-[calc(100vh-140px)] justify-start gap-4 overflow-hidden">
        ...
        <div className="grid grid-cols-2 gap-4">
          <Select 
            ...
            onChange={(e) => state.setPriceType(e.target.value as any)}
            disabled={isLoading}
        ...
        <div className="flex flex-col gap-2 mt-1 overflow-hidden flex-1">
          <div className="flex flex-col gap-1.5">
            <Select
              ...
              disabled={isLoading || state.isDishesLoading}
              className="h-11 text-xs"
            >
        ...
        <div className="flex justify-end gap-3 pt-3 mt-auto border-t border-brand-gray/10 shrink-0">
          ...
          <Button 
            variant="brand" 
            onClick={state.handleValidateAndSubmit} 
            isLoading={isLoading} 
            disabled={isLoading || !state.name.trim() || state.selectedDishes.length === 0}
