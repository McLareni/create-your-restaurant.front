    ...
    <Card className="p-5!">
      <div className="flex items-start justify-between mb-4 shrink-0 relative z-10">
        <div className="flex items-center gap-2">
          <div className="p-2 bg-brand-cream dark:bg-brand-mocha/50 rounded-lg text-brand-copper"><PackagePlus className="h-5 w-5" /></div>
      ...
      <div className="flex flex-col gap-1 mb-4 flex-1 relative z-0">
        {combo.dishes.map(dish => (
          <div key={dish.id} className="text-sm text-brand-gray dark:text-brand-gray/80 flex items-center gap-2">
            <span className="w-1 h-1 rounded-full bg-brand-gray/40 dark:bg-brand-gray/60 shrink-0"></span><span className="truncate">{dish.name}</span>
          </div>
        ))}
      </div>
      ...
      <div className="mt-auto border-t border-brand-gray/10 dark:border-brand-gray/20 pt-3 flex items-center justify-between relative z-0">
        <div className="flex flex-col">
          <span className="text-xs text-brand-gray dark:text-brand-gray/60 line-through">{calculateComboOriginalPrice(combo)} {t('menu.currency')}</span>
