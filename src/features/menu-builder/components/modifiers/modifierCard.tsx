    ...
    <div className="flex flex-col rounded-xl bg-white dark:bg-brand-mocha border border-brand-gray/10 shadow-xs transition-all">
      <div className="flex items-center justify-between p-3 sm:p-4 hover:bg-brand-cream/30 dark:hover:bg-brand-gray/5 transition-colors rounded-xl">
        ...
        <div className="flex items-center gap-1.5 shrink-0">
          <button onClick={onEditGroup} className="p-2 rounded-lg text-brand-gray hover:text-brand-copper hover:bg-brand-cream dark:hover:bg-brand-gray/10 transition-colors">
            <Pencil className="h-4 w-4" />
          </button>
          <button onClick={onDeleteGroup} className="p-2 rounded-lg text-brand-gray hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-500/10 transition-colors">
            <Trash2 className="h-4 w-4" />
          </button>
        </div>
      </div>

      <div className={`transition-all duration-300 ease-in-out origin-top ${isExpanded ? 'max-h-500 opacity-100 border-t border-brand-gray/10' : 'max-h-0 opacity-0 overflow-hidden border-t-0'}`}>
              ...
              <div key={option.id} className={`flex items-center justify-between p-3 rounded-lg border border-brand-gray/10 bg-brand-cream/10 dark:bg-brand-gray/5 hover:border-brand-copper/30 transition-colors ${!option.isAvailable ? 'opacity-60' : ''}`}>
                ...
                <div className="flex items-center gap-4 sm:gap-6">
                  ...
                  <div className="flex items-center gap-1">
                    <button onClick={() => onEditOption(option)} className="p-1.5 rounded-md text-brand-gray hover:text-brand-copper hover:bg-white transition-colors">
                      <Pencil className="h-3.5 w-3.5" />
                    </button>
                    <button onClick={() => onDeleteOption(option.id)} className="p-1.5 rounded-md text-brand-gray hover:text-red-500 hover:bg-white transition-colors">
