...
export const ModifiersTab = () => {
  ...
  const handleSaveGroup = () => {
    const formattedData = {
      ...
      options: editingGroup ? undefined : [], // При створенні опції пусті
    };
  ...
  const handleSaveOption = () => {
    ...
    if (editingOption) {
      newOptions = newOptions.map(opt => opt.id === editingOption.id ? { ...opt, ...formattedOption } : opt);
    } else {
      newOptions.push(formattedOption);
    }

    // Відправляємо PATCH запит на групу з новим масивом опцій
