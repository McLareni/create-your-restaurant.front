...
export const ModifiersTab = () => {
  ...
  const {
    ...
    handleSave,
    confirmDelete,
  } = useCrudModal<ModifierGroup, CreateModifierDTO>({
    initialFormData: INITIAL_FORM_DATA,
    createItem: createModifier,
    updateItem: updateModifier,
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      {modifiers.length === 0 ? (
        <EmptyState icon={<Layers />} title={t('menu.constructor.modifiers.emptyTitle')} description={t('menu.constructor.modifiers.emptyDesc')} actionLabel={t('menu.constructor.modifiers.addBtn')} onAction={onOpenCreate} />
      ) : (
        <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar pb-4">
          ...
          >
            {modifiers.map(mod => <ModifierCard key={mod.id} modifier={mod} onEdit={(item) => openEditModal(item, (i) => ({ ...i }))} onDelete={setDeleteId} />)}
          </div>
        </div>
      )}
