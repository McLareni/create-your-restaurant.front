...
export const CreateOrgForm = ({ state }: CreateOrgFormProps) => {
  ...
  const [sidebarPreview, setSidebarPreview] = useState(true);
  ...
  const [socialPlatform1, setSocialPlatform1] = useState<'instagram' | 'facebook' | 'telegram' | 'tiktok'>('instagram');
  const [socialPlatform2, setSocialPlatform2] = useState<'instagram' | 'facebook' | 'telegram' | 'tiktok'>('facebook');
  
  const { 
    formData, errors, isCheckingSlug, slugAvailable, isLoading, 
    handleChange, handleDaysChange, handleImageChange, handleSubmit 
  } = state;
    ...
    <div className="flex min-h-screen w-full items-start justify-center bg-bg-main text-text-main transition-colors duration-500 pt-12 pb-12 px-4 md:px-8 print:bg-white print:p-0">
      <div className="flex items-start justify-center w-full max-w-6xl p-2 gap-0">
        ...
        <div className="relative w-full max-w-xl rounded-3xl bg-bg-surface p-6 md:p-10 border border-border-main/80 shadow-2xl z-10
          ...
          [&_span.text-red-500]:text-red-600!
          [&_span.text-red-500]:font-bold!"
        >
          <button
            type="button"
          ...
          <div className="mb-8 flex flex-col items-center text-center w-full">
            ...
            <h1 className="text-xl md:text-2xl font-serif font-bold tracking-tight bg-linear-to-r from-text-main to-text-main/80 bg-clip-text text-transparent">{t('organization.create.title')}</h1>
            <p className="text-xs text-text-muted mt-1.5 font-medium">{t('organization.create.subtitle')}</p>
          </div>

          <form onSubmit={handleSubmit} className="space-y-5 w-full">
            <div className="space-y-4">
              <Input id="name" label={t('organization.create.nameLabel')} placeholder={t('organization.create.namePlaceholder')} value={formData.name || ''} onChange={(e) => handleChange('name', e.target.value)} error={errors.name} disabled={isLoading} autoComplete="organization" />
              
              <Input id="slug" label={t('organization.create.slugLabel')} hint={t('organization.create.slugHint')} value={formData.slug || ''} onChange={(e) => handleChange('slug', e.target.value)} error={errors.slug} disabled={isLoading} autoComplete="off" rightElement={
              ...
              <div className="grid grid-cols-2 gap-4">
                <Select id="type" label={t('organization.create.typeLabel')} value={formData.type || ''} onChange={(e) => handleChange('type', e.target.value)} error={errors.type}>
                ...
                <Select id="currency" label={t('organization.create.currencyLabel')} value={formData.currency || ''} onChange={(e) => handleChange('currency', e.target.value)} error={errors.currency}>
            ...
            <div className="border border-border-main/50 rounded-2xl overflow-hidden bg-bg-main/10 shadow-3xs group/sec">
              <button type="button" onClick={() => toggleSection('media')} className="w-full flex items-center justify-between p-4.5 text-xs font-bold uppercase tracking-wider text-text-main/80 hover:bg-bg-hover/30 hover:text-text-main transition-all cursor-pointer outline-none">
                <div className="flex items-center gap-2.5"><ImageIcon className="h-4 w-4 text-brand-copper/80 group-hover/sec:text-brand-copper transition-colors" /> <span>{t('organization.create.mediaTitle')}</span></div>
                  ...
                  <div className="p-4">
                    <div className="border border-dashed border-border-main/80 rounded-xl p-5 bg-bg-surface flex flex-col gap-2 hover:border-brand-copper/30 hover:bg-bg-main/5 transition-all relative">
                      <div className="flex items-center gap-4 w-full">
                        <div className="h-12 w-12 rounded-xl bg-bg-main border border-border-main flex items-center justify-center text-text-muted overflow-hidden shrink-0 shadow-inner">
                          {formData.imageUrl ? <img src={formData.imageUrl} className="h-full w-full object-cover" alt="Preview" /> : <ImageIcon className="h-5 w-5 stroke-[1.5]" />}
                        </div>
                        ...
                        <input type="file" accept="image/*" onChange={(e) => e.target.files?.[0] && handleImageChange(e.target.files[0])} className="absolute inset-0 opacity-0 cursor-pointer" />
                      </div>
                    </div>
                  </div>
                </div>
            ...
            <div className="border border-border-main/50 rounded-2xl overflow-hidden bg-bg-main/10 shadow-3xs group/sec">
              <button type="button" onClick={() => toggleSection('schedule')} className="w-full flex items-center justify-between p-4.5 text-xs font-bold uppercase tracking-wider text-text-main/80 hover:bg-bg-hover/30 hover:text-text-main transition-all ease-in-out duration-200 cursor-pointer outline-none">
                <div className="flex items-center gap-2.5"><Clock className="h-4 w-4 text-brand-copper/80 group-hover/sec:text-brand-copper transition-colors" /> <span>{t('organization.create.scheduleTitle')}</span></div>
              ...
              <div className={`grid transition-all duration-300 ease-in-out ${openSections.schedule ? 'grid-rows-[1fr] border-t border-border-main/40 opacity-100' : 'grid-rows-[0fr] opacity-0'}`}>
                <div className="overflow-hidden"><div className="p-5 space-y-5">
                      ...
                      {daysOfWeek.map((day) => {
                        ...
                        return (
                          <button
                            ...
                            className={`flex-1 flex flex-col items-center justify-center py-2.5 rounded-xl transition-all duration-200 cursor-pointer outline-none ${
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-2 gap-4 bg-bg-main/30 p-4 rounded-2xl border border-border-main/40">
                      <div className="flex flex-col gap-1">
                        ...
                        <Input id="workHoursStart" type="time" value={formData.workHoursStart || '10:00'} onChange={(e) => handleChange('workHoursStart', e.target.value)} error={errors.workHoursStart} className="text-center font-mono font-bold border-none! bg-bg-surface!" />
                      </div>
                      <div className="flex flex-col gap-1">
                        <span className="text-[9px] font-bold uppercase tracking-wider text-text-muted">{t('organization.create.closeLabel')}</span>
                        <Input id="workHoursEnd" type="time" value={formData.workHoursEnd || '22:00'} onChange={(e) => handleChange('workHoursEnd', e.target.value)} error={errors.workHoursEnd} className="text-center font-mono font-bold border-none! bg-bg-surface!" />
                      </div>
                    </div>
                  </div>
                </div></div>
            ...
            <div className="border border-border-main/50 rounded-2xl overflow-hidden bg-bg-main/10 shadow-3xs group/sec">
              <button type="button" onClick={() => toggleSection('address')} className="w-full flex items-center justify-between p-4.5 text-xs font-bold uppercase tracking-wider text-text-main/80 hover:bg-bg-hover/30 hover:text-text-main transition-all cursor-pointer outline-none">
                <div className="flex items-center gap-2.5"><MapPin className="h-4 w-4 text-brand-copper/80 group-hover/sec:text-brand-copper transition-colors" /> <span>{t('organization.create.locationTitle')}</span></div>
              ...
              <div className={`grid transition-all duration-300 ease-in-out ${openSections.address ? 'grid-rows-[1fr] border-t border-border-main/40 opacity-100' : 'grid-rows-[0fr] opacity-0'}`}>
                <div className="overflow-hidden"><div className="p-4 grid grid-cols-1 sm:grid-cols-3 gap-3">
                  <Input id="city" label={t('organization.create.cityLabel')} placeholder={t('organization.create.cityPlaceholder')} value={formData.city || ''} onChange={(e) => handleChange('city', e.target.value)} error={errors.city} autoComplete="address-level2" />
                  <Input id="street" label={t('organization.create.streetLabel')} placeholder={t('organization.create.streetPlaceholder')} value={formData.street || ''} onChange={(e) => handleChange('street', e.target.value)} error={errors.street} autoComplete="address-line1" />
                  <Input id="building" label={t('organization.create.buildingLabel')} placeholder={t('organization.create.buildingPlaceholder')} value={formData.building || ''} onChange={(e) => handleChange('building', e.target.value)} error={errors.building} autoComplete="off" />
                </div></div>
              </div>
            </div>
            ...
            <div className="border border-border-main/50 rounded-2xl overflow-hidden bg-bg-main/10 shadow-3xs group/sec">
              <button type="button" onClick={() => toggleSection('socials')} className="w-full flex items-center justify-between p-4.5 text-xs font-bold uppercase tracking-wider text-text-main/80 hover:bg-bg-hover/30 hover:text-text-main transition-all cursor-pointer outline-none">
                <div className="flex items-center gap-2.5"><Share2 className="h-4 w-4 text-brand-copper/80 group-hover/sec:text-brand-copper transition-colors" /> <span>{t('organization.create.socialsTitle')}</span></div>
              ...
              <div className={`grid transition-all duration-300 ease-in-out ${openSections.socials ? 'grid-rows-[1fr] border-t border-border-main/40 opacity-100' : 'grid-rows-[0fr] opacity-0'}`}>
                <div className="overflow-hidden"><div className="p-4 space-y-4">
                  <Input id="phone" label={t('organization.create.phoneLabel')} placeholder={t('organization.create.phonePlaceholder')} value={formData.phone || ''} onChange={(e) => handleChange('phone', e.target.value)} error={errors.phone} autoComplete="tel" />
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="flex gap-2.5 items-center">
                      <div className="w-1/3">
                        <Select value={socialPlatform1} onChange={(e) => { handleChange(socialPlatform1, ''); setSocialPlatform1(e.target.value as any); }}>
                      ...
                      <div className="flex-1">
                        <Input id={socialPlatform1} placeholder="username" value={(formData[socialPlatform1] as string) || ''} onChange={(e) => handleChange(socialPlatform1, e.target.value)} error={errors[socialPlatform1]} autoComplete="off" />
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="flex gap-2.5 items-center">
                      <div className="w-1/3">
                        <Select value={socialPlatform2} onChange={(e) => { handleChange(socialPlatform2, ''); setSocialPlatform2(e.target.value as any); }}>
                          <option value="facebook">Facebook</option>
                          <option value="instagram">Instagram</option>
                          <option value="telegram">Telegram</option>
                          <option value="tiktok">TikTok</option>
                      ...
                      <div className="flex-1">
                        <Input id={socialPlatform2} placeholder="username" value={(formData[socialPlatform2] as string) || ''} onChange={(e) => handleChange(socialPlatform2, e.target.value)} error={errors[socialPlatform2]} autoComplete="off" />
                      </div>
                    </div>
                  </div>
                </div></div>
            ...
            <div className="pt-3 w-full">
              <Button variant="brand" type="submit" className="w-full h-12 rounded-xl text-sm font-bold shadow-md transition-all active:scale-98" isLoading={isLoading} disabled={isLoading || isCheckingSlug || slugAvailable === false}>
