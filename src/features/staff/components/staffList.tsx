  ...
  return (
    <div className="flex h-full flex-col">
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between mb-6 pb-4 border-b border-brand-gray/10 shrink-0">
        <div>
          <h2 className="text-xl font-semibold text-brand-espresso">{t('staff.title')}</h2>
          <p className="text-sm text-brand-gray mt-1">{t('staff.subtitle')}</p>
        </div>
        <div className="flex gap-3 items-center w-full md:w-auto">
          <div className="relative w-full md:w-64">
            <Search className="absolute left-3 top-3.5 h-4 w-4 text-brand-gray/60" />
            <input type="text" placeholder={t('staff.searchPlaceholder')} value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="h-11 w-full rounded-full border border-brand-gray/30 bg-white pl-9 pr-4 text-sm text-brand-espresso outline-none transition-colors focus:border-brand-copper focus:ring-1 focus:ring-brand-copper" />
          </div>
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={editingMember ? t('staff.modal.editTitle') : t('staff.modal.createTitle')}>
        <div className="flex flex-col gap-5">
          ...
          <div className="flex items-center justify-between border-t border-brand-gray/10 pt-4 mt-2">
            <span className="text-sm font-medium text-brand-espresso">{t('staff.modal.statusLabel')}</span>
